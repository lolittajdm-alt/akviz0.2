    ...
    <div style={iosContainer}>
      ...
      {showTopFields&&(
        <div style={iosCard}>
          ...
          <div style={{display:"flex",gap:"0.5rem"}}>
            <input name="subdivision" value={form.subdivision} onChange={handleChange} style={iosInput} placeholder="Наприклад 3 ОТБр" />
          ...
          <div style={{display:"flex",gap:"0.5rem"}}>
            <input name="position" value={form.position} onChange={handleChange} style={iosInput} placeholder="Наприклад МВГ Халк" />
          ...
          <div style={{display:"flex",gap:"0.5rem"}}>
            <input name="location" value={form.location} onChange={handleChange} style={iosInput} placeholder="Наприклад Іванівка" />
      ...
      <div style={iosCard}>
        ...
        <div style={{display:"flex",gap:"0.5rem"}}>
          {["Ворожий","Свій","Нейтральний"].map(s=>(
            <button key={s} onClick={()=>selectSide(s)} style={{...iosButton,background:form.side===s?"#32D74B":"#EBEBF5",color:form.side===s?"#fff":"#1C1C1E"}}>{s}</button>
          ))}
        </div>
      </div>
      ...
      {!form.noIssue&&(
        <div style={iosCard}>
          ...
          <div style={{display:"flex",gap:"0.5rem",alignItems:"center"}}>
            <input name="targetNumber" value={form.targetNumber} onChange={handleChange} style={iosInput} placeholder="Введіть номер цілі" />
            <button onClick={()=>setForm(f=>({...f,noIssue:true,targetNumber:""}))} style={{...iosButton,height:"44px",whiteSpace:"nowrap"}}>Без видачі</button>
          </div>
          {form.targetNumber.trim()===""&&<div style={{color:"#FF3B30",fontSize:"0.75rem",marginTop:"0.25rem"}}>Вкажіть номер або оберіть «Без видачі»</div>}
        </div>
      )}
      ...
      <div style={iosCard}>
        ...
        <div style={{display:"flex",gap:"0.5rem"}}>
          {namesList.map(n=>(
            <button key={n} onClick={()=>selectName(n)} style={{...iosButton,background:form.name===n?"#32D74B":"#EBEBF5",color:form.name===n?"#fff":"#1C1C1E"}}>{n}</button>
          ))}
        </div>
      </div>
      ...
      <div style={iosCard}>
        ...
        <div style={{display:"flex",gap:"0.5rem",alignItems:"center"}}>
          <button onClick={()=>changeQuantity(-1)} style={{...iosButton,padding:"0.5rem 0.9rem",background:"#FF375F",color:"#fff"}} disabled={form.quantity<=1}>–</button>
          <span style={{fontSize:"1.2rem",width:"2rem",textAlign:"center"}}>{form.quantity}</span>
          <button onClick={()=>changeQuantity(1)} style={{...iosButton,padding:"0.5rem 0.9rem",background:"#32D74B",color:"#fff"}}>+</button>
        </div>
      </div>
      ...
      <div style={iosCard}>
        ...
        <input type="text" inputMode="numeric" value={form.azimuth} onChange={onFieldNumeric("azimuth")} style={{...iosInput,border:form.azimuth.trim()===""?"1px solid #FF3B30":"1px solid transparent"}} placeholder="0–359" />
        {form.azimuth.trim()===""&&<div style={{color:"#FF3B30",fontSize:"0.75rem"}}>Поле має бути заповненим!</div>}
      </div>

      {/* Курс */}
      <div style={iosCard}>
        <label style={iosLabel}>Курс, °</label>
        <input type="text" inputMode="numeric" value={form.course} onChange={onFieldNumeric("course")} style={iosInput} placeholder="0–359" />
      </div>

      {/* Відстань */}
      <div style={iosCard}>
        <label style={iosLabel}>Відстань, м*</label>
        <input type="text" inputMode="numeric" value={form.distance} onChange={onFieldNumeric("distance")} style={{...iosInput,border:form.distance.trim()===""||errors.distance?"1px solid #FF3B30":"1px solid transparent"}} placeholder="Відстань до цілі" />
        {errors.distance&&<div style={{color:"#FF3B30",fontSize:"0.75rem"}}>Поле має бути заповненим!</div>}
      </div>

      {/* Висота */}
      <div style={iosCard}>
        <label style={iosLabel}>Висота, м*</label>
        <input type="text" inputMode="numeric" value={form.height} onChange={onFieldNumeric("height")} style={{...iosInput,border:form.height.trim()===""||errors.height?"1px solid #FF3B30":"1px solid transparent"}} placeholder="Висота" />
        {errors.height&&<div style={{color:"#FF3B30",fontSize:"0.75rem"}}>Поле має бути заповненим!</div>}
      </div>
      ...
      <div style={iosCard}>
        ...
        <div style={{display:"flex",gap:"0.5rem",marginBottom:"0.5rem"}}>
          {["Акустично","Візуально"].map(m=>(
            <button key={m} onClick={()=>toggleDetection(m)} style={{...iosButton,flex:1,background:form.detectionMethods.includes(m)?"#32D74B":"#EBEBF5",color:form.detectionMethods.includes(m)?"#fff":"#1C1C1E"}}>{m}</button>
          ))}
        </div>
        <button onClick={()=>toggleDetection("Із застосуванням приладів спостереження")} style={{...iosButton,background:form.detectionMethods.includes("Із застосуванням приладів спостереження")?"#32D74B":"#EBEBF5",color:form.detectionMethods.includes("Із застосуванням приладів спостереження")?"#fff":"#1C1C1E"}}>Із застосуванням приладів спостереження</button>
      </div>
      ...
      <div style={iosCard}>
        ...
        <div style={{display:"flex",gap:"0.5rem"}}>
          {["Уражено","Не уражено","Достовірно невизначено"].map(r=>(
            <button key={r} onClick={()=>setForm(f=>({...f,result:r}))} style={{...iosButton,background:form.result===r?"#32D74B":"#EBEBF5",color:form.result===r?"#fff":"#1C1C1E"}}>{r}</button>
          ))}
        </div>
      </div>
      ...
      <div style={iosCard}>
        ...
        <textarea name="description" value={form.description} onChange={handleChange} placeholder="Додаткова інформація" style={{...iosInput,height:"80px"}} />
      </div>

      {/* Кнопки Копіювати/WhatsApp */}
      <div style={{display:"flex",gap:"0.5rem",marginBottom:"2rem"}}>
        <button onClick={copyReport} style={iosButton}>Копіювати</button>
        <button onClick={openWhatsApp} style={{...iosButton,background:"#34C759",color:"#fff"}}>WhatsApp</button>
      </div>
